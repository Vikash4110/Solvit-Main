// components/admin/ApplicationsPage.jsx - Modern, Responsive Counselor Applications Dashboard
...
const ApplicationsPage = () => {
  ...
  const filteredApplications = useMemo(() => {
    return applications.filter((app) => {
      ...
      const matchSpecialization = app.specialization?.some((spec) =>
        spec.toLowerCase().includes(q)
      );
      const matchId = app._id?.toString().includes(q);
      return matchName || matchEmail || matchSpecialization || matchId;
    });
  ...
  const stats = useMemo(() => {
    return {
      ...
      pending: applications.filter(
        (app) => app.application?.applicationStatus === 'pending'
      ).length,
      approved: applications.filter(
        (app) => app.application?.applicationStatus === 'approved'
      ).length,
      rejected: applications.filter(
        (app) => app.application?.applicationStatus === 'rejected'
      ).length,
    };
  }, [applications]);
  ...
  const getStatusLabel = (status) => {
    switch (status) {
      ...
      case 'rejected':
        return 'Rejected';
      default:
        return status ? status.replace(/_/g, ' ') : 'Not Submitted';
    }
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-500 dark:text-neutral-400">
              Review, verify, and approve counselor onboarding applications
            </p>
          </div>
        </div>
        ...
        <CardContent className="p-0">
          {loading ? (
            <div className="flex flex-col items-center justify-center py-20 text-slate-500 dark:text-neutral-400">
              ...
              <p className="text-sm font-medium">Loading counselor applications...</p>
            </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3.5 px-2.5 text-center whitespace-nowrap">
                          <Badge
                            variant="outline"
                            className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 text-xs font-semibold rounded-full border shadow-xs whitespace-nowrap ${getStatusColor(
                              appStatus
                            )}`}
                          >
                            {getStatusIcon(appStatus)}
                            <span>{getStatusLabel(appStatus)}</span>
                          </Badge>
                        </td>
