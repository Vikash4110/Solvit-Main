...
const BlogManagement = () => {
  ...
  const fetchCounselorBlogs = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}${API_ENDPOINTS.BLOGS_COUNSELOR_MY_BLOGS}?${params}`, {
        headers: {
  ...
  const handleUpdateBlog = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}${API_ENDPOINTS.BLOGS_UPDATE}/${selectedBlog._id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
            ...
            <div>
              <h1 className="text-4xl font-bold text-gray-900 mb-2">
                📝 {currentView === 'list' ? 'Blog Management' : 
                     currentView === 'create' ? 'Create New Blog' :
                     currentView === 'edit' ? 'Edit Blog' : 'View Blog'}
              </h1>
              <p className="text-xl text-gray-600">
                {currentView === 'list' ? 'Manage your blog posts and build your reputation' :
                 currentView === 'create' ? 'Share your expertise with your audience' :
                 currentView === 'edit' ? 'Update your blog content' : 'Review your blog post'}
              </p>
            </div>
          </div>
            ...
            >
              ...
              {loading ? (
                ...
                <div className="text-center py-20">
                  ...
                  <p className="text-gray-600 mb-6">Start creating your first blog post to share your expertise</p>
                  <button
          ...
          {(currentView === 'create' || currentView === 'edit') && (
            ...
            >
              <div className="bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl border border-white/50 p-8">
                <form onSubmit={currentView === 'create' ? handleCreateBlog : handleUpdateBlog} className="space-y-6">
                  {/* Title */}
                  <div>
                    ...
                    >
                      {formLoading ? (
                        ...
                        <>
                          ...
                          {currentView === 'create' ? 
                            (blogData.status === 'published' ? 'Publish Blog' : 'Save Draft') :
                            'Update Blog'
                          }
                        </>
                      )}
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center space-x-4">
                      <span className={`px-3 py-1 rounded-full text-sm font-bold ${
                        selectedBlog.status === 'published' ? 'bg-green-100 text-green-700' :
                        selectedBlog.status === 'draft' ? 'bg-yellow-100 text-yellow-700' :
                        'bg-gray-100 text-gray-700'
                      }`}>
