  ...
  const fetchBlog = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}${API_ENDPOINTS.BLOGS_GET_BY_SLUG}/${slug}`,
        {
          headers: {
      ...
      if (data.success) {
        ...
        if (isAuthenticated) {
          ...
          const hasLiked = data.data.blog.likes?.some(
            (like) => like.user === userId && like.userType === (userType === 'client' ? 'Client' : 'Counselor')
          );
          setLiked(hasLiked);
        }
      } else {
            ...
            <div className="flex items-center justify-center space-x-2 text-blue-700">
              ...
              <span className="font-medium">
                <Link to="/login" className="text-blue-600 hover:text-blue-800 underline">
                  Login
                </Link> to like and comment on this blog
              </span>
            </div>
          </motion.div>
        )}
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 pb-8 border-b border-gray-200">
              ...
              <div className="flex items-center space-x-6 text-sm text-gray-600">
                ...
                <div className="flex items-center space-x-2">
                  <button
                    onClick={isAuthenticated ? handleLike : () => handleAuthRequired("like this blog")}
