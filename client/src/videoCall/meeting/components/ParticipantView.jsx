...
function ParticipantsViewer({ isPresenting }) {
  ...
  const participantIds = useMemo(() => {
    const pinnedParticipantId = [...pinnedParticipants.keys()].filter((participantId) => {
      return participantId !== localParticipant.id;
    });
    const regularParticipantIds = [...participants.keys()].filter((participantId) => {
      return (
    ...
    const ids = [localParticipant.id, ...pinnedParticipantId, ...regularParticipantIds].slice(
      0,
      isPresenting ? 6 : 16
    );
  ...
  return <MemoizedParticipantGrid participantIds={participantIds} isPresenting={isPresenting} />;
}

const MemorizedParticipantView = React.memo(ParticipantsViewer, (prevProps, nextProps) => {
