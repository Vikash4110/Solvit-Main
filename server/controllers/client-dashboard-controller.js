...
const canCancelBooking = (booking) => {
  ...
  const sessionStart = dayjs(
    `${booking.slot?.date?.split("T")[0]} ${booking.slot?.startTime}`,
    "YYYY-MM-DD hh:mm A"
  );
...
const canJoinSession = (booking) => {
  ...
  const sessionEnd = dayjs(
    `${booking.slot?.date?.split("T")[0]} ${booking.slot?.endTime}`,
    "YYYY-MM-DD hh:mm A"
  );
...
export const getBookings = wrapper(async (req, res) => {
  ...
  const pipeline = [
      ...
      $addFields: {
        ...
        slotDateTime: {
          $dateFromString: {
            dateString: {
              $concat: [
                { $substr: ["$slot.date", 0, 10] }, // YYYY-MM-DD
                " ",
                "$slot.startTime", // e.g. "10:00 AM"
              ],
            },
            format: "%Y-%m-%d %I:%M %p", // parse combined date & time
          },
        },
      },
    },
    ...
    {
      $sort: {
        slotDateTime: filter === "history" ? -1 : 1,
      },
    },
    {
      $skip: skip,
  ...
  const enrichedBookings = bookings.map((booking) => {
    ...
    return {
      ...
      cancellationDeadline: booking.date
        ? dayjs(booking.date).subtract(24, "hour").toISOString()
        : null,
    };
  });
